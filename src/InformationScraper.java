...
public class InformationScraper {
	...
	/**
	 * getPlanetNames parses the HTML from the given website to retrieve planet names
	 * and adds randomized suffixes on random
	 * @param count star system size
	 * @return the list of all body names
	 */
	...
	/**
	 * getMap prints out a visual star system map (ASCII text) with the given details
	 * @param systemSize star system size
	...
	/**
	 * getLink parses the HTML from the appropriate website with the given details.
	 * A double array is used to hold all links of ASCII texts. Mapping ranges from
	 * 1 planet to 9 planet mapping
	 * @param systemSize star system size
	 * @param currentLocation current location
	 * @returns the map appropriate link
	 */
	...
	private String getLink(int systemSize, int currentLocation) {
		...
		return allURLMaps[(systemSize - 2)][currentLocation];
	}
}
